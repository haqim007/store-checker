...
class UpdateStoreLocationUseCase @Inject constructor(
    private val repository: IStoreRepository
) {
    suspend operator fun invoke(store: Store, newLocation: LatLng){
        return repository.updateStoreLocation(store, newLocation)
    }
}